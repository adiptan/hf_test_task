...
# Log config
logging.basicConfig(filename='importer.log', format='%(asctime)s - %(message)s', level=logging.INFO)
...
arg_parser.add_argument("--token", dest="token", help="Api token.")
...
if __name__ == '__main__':
    ...
    for applicant_data in loaded_data:
        ...
        vacancy_status_id = common_func.get_applicant_vacancy_status_id(get_vacancy_statuses.api_get_method()['items'],
        ...
        add_applicant_to_vacancy = hf_api.PostMethod(args.token,
                                                     ...
                                                     applicant_data["Комментарий"], file_id))
        print(add_applicant_to_vacancy.add_new_applicant())
        logging.info(f'Applicant {applicant_data["ФИО"]} is fully processed.')
